...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ДОГОВОР ЗАЙМА № 17\nЗаймодавец и Заёмщик заключили настоящий договор", "Договор займа"),
...
# --- расширенный справочник видов документов --------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ("АПЕЛЛЯЦИОННОЕ ОПРЕДЕЛЕНИЕ\nСудебная коллегия по гражданским делам",
         "Апелляционное определение"),
        ("КОММЕРЧЕСКОЕ ПРЕДЛОЖЕНИЕ\nПредлагаем вам поставку оборудования",
         "Коммерческое предложение"),
        ("СОГЛАШЕНИЕ О НЕРАЗГЛАШЕНИИ\nСтороны обязуются", "Соглашение о неразглашении"),
        ("ПИСЬМО О НАМЕРЕНИЯХ\nнамерено заключить договор", "Письмо о намерениях"),
        ("УСТАНОВОЧНАЯ СПРАВКА\nв отношении Петрова С.А.", "Справка установочная"),
        ("СИТУАЦИОННАЯ СПРАВКА\nпо состоянию на 1 марта", "Справка ситуационная"),
        ("АКТ ВЫПОЛНЕННЫХ РАБОТ № 7\nРаботы выполнены в полном объёме",
         "Акт выполненных работ"),
        ("ТЕХНИЧЕСКОЕ ЗАДАНИЕ\nТребования к системе", "Техническое задание"),
        ("ТРУДОВОЙ ДОГОВОР\nРаботник обязуется выполнять", "Трудовой договор"),
        ("ВЫПИСКА ИЗ ЕГРН\nКадастровый номер", "Выписка из ЕГРН"),
        ("ПРОТОКОЛ ОБЩЕГО СОБРАНИЯ участников общества", "Протокол общего собрания"),
        ("БИЗНЕС-ПЛАН\nплан развития проекта", "Бизнес-план"),
    ],
)
def test_extended_document_types(matcher: DocumentTypeMatcher, text: str, expected: str) -> None:
    best = select_document_type(matcher.match(text))
    assert best is not None, text
    assert best.value == expected


def test_type_named_in_filename_is_trusted(matcher: DocumentTypeMatcher) -> None:
    """Вид документа, написанный в имени файла, — сильное свидетельство.

    Его написал человек, который документ видел; в тексте самого акта слова
    «апелляционное определение» может не быть.
    """
    best = select_document_type(
        matcher.match(
            "Судебная коллегия рассмотрела дело по жалобе Карелиной Н.В.",
            filename="Дело 33-52030_224 определение суда апелляционной инстанции.doc",
        )
    )

    assert best is not None
    assert best.value == "Апелляционное определение"


def test_case_number_is_read_from_filename() -> None:
    """Номер дела из имени файла попадает в реквизиты, а не в текст имени."""
    grouped = extract_identifiers("Дело 33-52030_224 определение суда")
    assert grouped["case_number"][0].value == "33-52030_224"


def test_dictionary_covers_business_documents() -> None:
    """Справочник охватывает не только суды, но и деловой оборот."""
    names = {entry["canonical_name"] for entry in load_document_types()}
    required = {
        "Коммерческое предложение", "Соглашение о неразглашении", "Письмо о намерениях",
        "Техническое задание", "Устав", "Протокол общего собрания", "Выписка из ЕГРЮЛ",
        "Выписка из ЕГРН", "Акт выполненных работ", "Смета", "Гарантийное письмо",
        "Справка установочная", "Справка ситуационная", "Аналитическая записка",
        "Резюме проекта", "Бизнес-план", "Финансовая модель", "Трудовой договор",
        "Должностная инструкция", "Страховой полис", "Апелляционное определение",
        "Кассационное определение", "Договор купли-продажи", "Договор аренды",
    }
    assert required - names == set()
    assert len(names) >= 100, f"видов документов в справочнике: {len(names)}"
