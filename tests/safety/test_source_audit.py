...
#: «scrub.py» подменяет пользовательский файл очищенным — но только когда
#: человек прямо велел снять метаданные и подтвердил замену. Это единственная
#: операция программы, меняющая сам файл, и она проходит через atomic replace.
OS_REPLACE_ALLOWED = {"config.py", "scrub.py"}
...
#: в её папке logs — пользовательских файлов он не касается. «scrub.py»
#: убирает собственный временный файл очистки и по прямой команде человека
#: заменяет исходный файл очищенным: снятие метаданных иначе невозможно.
DELETE_ALLOWED = {
    "temp_cleanup.py",
    "config.py",
    "rename.py",
