...
### Обновление и запрет сети

Раздел 3 ТЗ запрещает автоматическое обновление и любую сетевую
функциональность. Заказчик при этом попросил обновление из программы. Конфликт
решён разделением на две программы:

| Программа | Сеть | Пользовательские документы |
|---|---|---|
| `docrenamer` — обработка документов | нет, проверяется аудитом при каждой сборке | читает |
| `docrenamer_updater` — обновление | да, только HTTPS к странице релизов | не открывает никогда |

Главное окно не выполняет сетевых обращений: по нажатию кнопки оно запускает
отдельный исполняемый файл `DocRenamerUpdate.exe`. Загруженный установщик
проверяется по SHA-256 из опубликованного `SHA256SUMS.txt`; файл с несовпавшей
суммой удаляется и не запускается.

Аудит офлайн-режима нацелен на `src/docrenamer` — то есть на программу,
которая работает с документами. Отдельный тест следит за тем, что она не
импортирует пакет обновления.
...
### Правила именования, добавленные при приёмке

| Правило | Где реализовано |
|---|---|
| Осмысленное имя, данное человеком, не переписывается; допустимо только добавление даты в начале | `naming/builder.py`: `is_meaningful_stem`, `build_preserved_name` |
| Файлы, отличающиеся только номером, — части одного документа; номер сохраняется | `extractors/series.py`, `Pipeline.postprocess` |
| Недостающие реквизиты тома берутся у соседнего тома с явным указанием источника | `Pipeline._share_series_facts` |
