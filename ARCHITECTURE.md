...
cli.py / gui.py           тонкие оболочки, бизнес-логики не содержат
                          логика представления вынесена в presentation.py,
                          preview.py и browsing.py — они не зависят от Tkinter
                          и проверяются там, где графики нет
        │
        ▼
...
### Имя строится в два прохода

| Проход | Что делает | Модуль |
|---|---|---|
| Первый | собирает имя из найденных фактов | `naming/builder.py` |
| Второй | перечитывает результат независимыми проверками | `naming/review.py` |

Второй проход не знает, как собиралось имя, — он видит только готовую строку и
сегменты. Поэтому он ловит ошибки первого: две даты, повтор одного и того же
слова, случайное число, дату, принятую за номер, строчную букву в начале.
Мелочь исправляется сама, при серьёзной ошибке имя не предлагается вовсе.
...
| Документ | О чём |
|---|---|
| `docs/РАЗРАБОТЧИКУ.md` | карта модулей, поток данных, рецепты изменений, коды состояния |
| `docs/РУКОВОДСТВО.md` | то же самое глазами пользователя |
