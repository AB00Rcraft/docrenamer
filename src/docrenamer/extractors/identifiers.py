...
COURT_CASE_RE = re.compile(
    # Номер дела пишут и через дробь, и через дефис, и через подчёркивание:
    # «2-1234/2026», «33-52030_224». Слово «дело» рядом обязательно — иначе
    # под шаблон попадают телефоны и диапазоны.
    r"(?:дел[оаеу]\D{0,12})(?<![А-ЯЁA-Za-z\d])(\d{1,3}-\d{1,7}[/_-]\d{2,4})",
    re.IGNORECASE,
)
